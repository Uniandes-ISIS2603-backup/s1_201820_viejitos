(function (ng) {
    ...
    mod.controller('citasClienteNewCtrl', ['$scope', '$http', '$state', '$rootScope',
        /**
         * @ngdoc controller
        ...
        function ($scope, $http, $state, $rootScope) {
            ...
            $scope.createCita = function () {
                $http.post("api/clientes/"+ sessionStorage.getItem("id")+"/citas", $scope.data).then(function (response) {
                    $state.go('citasCList', {citaId: response.data.id}, {reload: true});
                });
            };
        }
    ]);
